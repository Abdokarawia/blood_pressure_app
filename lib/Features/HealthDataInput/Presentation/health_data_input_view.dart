...
class _HealthDataInputScreenState extends State<HealthDataInputScreen> {
  ...
  late final Map<String, GoalCategory> _titleToCategory =
  _categoryToTitle.map((key, value) => MapEntry(value, key));

  @override
  void initState() {
    super.initState();
    // Load health data and goals when the screen initializes
  ...
  @override
  void dispose() {
    // Reset controllers to defaults when screen is disposed
    context.read<HealthDataCubit>().resetToDefaults();
    super.dispose();
  }
  ...
  double _calculateGoalCaloricDeficit() {
    ...
    double goalWeightToLose =
        double.tryParse(cubit.goalWeightLossController.text) ?? 0;
    return 7700 * goalWeightToLose;
  }

  Color _getProgressColor(double? percentage) {
    if (percentage == null) return Colors.grey; // No goal set
  ...
  String _getProgressStatus(double? percentage) {
    if (percentage == null) return 'No goal set';
  ...
  Widget _buildHealthDataCard({
    ...
    final goalTarget = category != null ? cubit.getTargetForCategory(category) : null;

    // Calculate percentage only if goal is set
    final double? percentage = goalTarget != null && goalTarget > 0
        ? (currentValue / goalTarget * 100).clamp(0, 100).toDouble()
        : null;

    final progressColor = _getProgressColor(percentage);
    final isGoalSet = percentage != null;
                            ...
                            child: ListView(
                              ...
                              children: [
                                ...
                                _buildHealthDataCard(
                                  ...
                                  icon: Iconsax.health,
                                  color: Colors.purple,
                                  controller: context.read<HealthDataCubit>().stepsController,
                                  hint: 'Steps taken',
                                  keyboardType: TextInputType.number,
                                ...
                                _buildHealthDataCard(
                                  ...
                                  color: Colors.blue,
                                  controller: context.read<HealthDataCubit>().distanceController,
                                  hint: 'Distance covered',
                                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                  validator: (value) => _validateNumber(value, 'distance'),
                                  suffix: 'km',
                                ),
                                ...
                                _buildHealthDataCard(
                                  ...
                                  icon: Iconsax.moon,
                                  color: Colors.indigo,
                                  controller: context.read<HealthDataCubit>().sleepHoursController,
                                  hint: 'Hours slept',
                                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
